...
public class BearerTokenFilter implements ContainerRequestFilter {
    ...
    private boolean isUriUnProtected(String path, String method) {
        ...
        return listContains(unProtectedUris, new Pair(path, method));
    }

    private boolean listContains(ArrayList<Pair> uris, Pair uri) {
        for (Pair unprotectedUri : uris) {
            if (unprotectedUri.getMethod().equals(uri.getMethod())) {
                return true;
            }
        }
        return false;
